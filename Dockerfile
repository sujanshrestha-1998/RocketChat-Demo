...
# Install Node 22.14.0 (specific version Rocket.Chat wants)
...
# Clone Rocket.Chat repo and checkout tag
WORKDIR /app
RUN git clone --branch 7.7.1 https://github.com/RocketChat/Rocket.Chat.git .

# Install dependencies and build packages at root
RUN yarn install
RUN yarn build

# Build Meteor bundle inside apps/meteor
